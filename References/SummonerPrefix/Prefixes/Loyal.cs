...
// Type: SummonerPrefix.Prefixes.Loyal
...
public class Loyal : ModPrefix
{
  ...
  public virtual void SetStats(
    ...
    ref int critBonus)
  {
    damageMult = Loyal.MinionSlotAndDmgMult;
  }

  public virtual void ModifyValue(ref float valueMult) => valueMult *= 1.6f;

  public virtual void Apply(Item item) => item.global().MinionSlotMult = Loyal.MinionSlotAndDmgMult;
  ...
  public virtual IEnumerable<TooltipLine> GetTooltipLines(Item item)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
    TooltipLine tl = new TooltipLine(((ModType) loyal).Mod, "PrefixDescription", loyal.AdditionalTooltip.Value)
    {
      IsModifier = true,
      IsModifierBad = false
    };
    tl.Replace("{A}", (Loyal.MinionSlotAndDmgMult - 1f).ToPercent().ToString());
    Projectile projectile = new Projectile();
    projectile.SetDefaults(item.shoot);
    tl.Replace("{B}", Math.Round((double) projectile.minionSlots * ((double) Loyal.MinionSlotAndDmgMult - 1.0), 2).ToString());
    tl.Replace("{C}", Loyal.Crit.ToPercent().ToString());
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = tl;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
