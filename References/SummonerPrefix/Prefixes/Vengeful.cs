...
// Type: SummonerPrefix.Prefixes.Vengeful
...
public class Vengeful : ModPrefix
{
  ...
  {
    int num = (int) Math.Ceiling((double) item.damage * (double) Vengeful.ExtraTagDamage);
    if (num <= 0)
      return;
  ...
  public virtual IEnumerable<TooltipLine> GetTooltipLines(Item item)
  {
    ...
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = -1;
    TooltipLine tl = new TooltipLine(((ModType) vengeful).Mod, "PrefixDescription", vengeful.AdditionalTooltip.Value)
    {
      IsModifier = true,
      IsModifierBad = false
    };
    tl.Replace("{A}", Vengeful.ExtraTagDamage.ToPercent().ToString());
    tl.Replace("{B}", ((int) Math.Ceiling((double) item.damage * (double) Vengeful.ExtraTagDamage)).ToString());
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E2__current = tl;
    // ISSUE: reference to a compiler-generated field
    this.\u003C\u003E1__state = 1;
