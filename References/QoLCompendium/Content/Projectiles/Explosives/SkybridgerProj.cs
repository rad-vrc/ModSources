...
// Type: QoLCompendium.Content.Projectiles.Explosives.SkybridgerProj
...
{
  public virtual string Texture => "QoLCompendium/Content/Items/Tools/Explosives/Skybridger";
  ...
  public virtual void OnKill(int timeLeft)
  {
    ...
    if (Main.netMode == 1)
      return;
    for (int index1 = Main.maxTilesX / 2; index1 < Main.maxTilesX; ++index1)
    {
      for (int index2 = -90; index2 <= 0; ++index2)
      {
        int x = index1;
    ...
    for (int index3 = Main.maxTilesX / 2; index3 > 0; --index3)
    {
      for (int index4 = -90; index4 <= 0; ++index4)
      {
        int x = index3;
