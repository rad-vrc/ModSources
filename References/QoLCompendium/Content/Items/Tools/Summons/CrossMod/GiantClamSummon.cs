...
// Type: QoLCompendium.Content.Items.Tools.Summons.CrossMod.GiantClamSummon
...
public class GiantClamSummon : ModItem
{
  ...
  {
    ItemID.Sets.SortingPriorityBossSpawns[this.Type] = 2;
  ...
  {
    ModBiome modBiome;
    return ModConditions.calamityLoaded && ModConditions.calamityMod.TryFind<ModBiome>("SunkenSeaBiome", ref modBiome) && modBiome != null && Main.LocalPlayer.InModBiome(modBiome) && ModConditions.DownedDesertScourge.IsMet() && !NPC.AnyNPCs(Common.GetModNPC(ModConditions.calamityMod, "GiantClam"));
  }
    ...
    float knockback)
  {
    ...
    Projectile.NewProjectile(player.GetSource_ItemUse(this.Item, (string) null), vector2, Vector2.Zero, ModContent.ProjectileType<NPCSpawner>(), 0, 0.0f, ((Entity) player).whoAmI, (float) Common.GetModNPC(ModConditions.calamityMod, "GiantClam"), 0.0f, 0.0f);
