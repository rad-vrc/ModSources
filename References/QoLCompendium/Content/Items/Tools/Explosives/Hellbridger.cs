...
// Type: QoLCompendium.Content.Items.Tools.Explosives.Hellbridger
...
public class Hellbridger : ModItem
{
  ...
  public virtual void SetDefaults()
  {
    ...
    this.Item.shoot = ModContent.ProjectileType<HellbridgerProj>();
    ...
    float knockback)
  {
    ...
    if (player.altFunctionUse == 2)
      Projectile.NewProjectile(player.GetSource_ItemUse(((EntitySource_ItemUse) source).Item, (string) null), mouseWorld, Vector2.Zero, ModContent.ProjectileType<HellbridgerSingleProj>(), 0, 0.0f, ((Entity) player).whoAmI, 0.0f, 0.0f, 0.0f);
    else
  ...
  public void HandleShadow(Player player)
  {
    ...
    for (int index = -100; index <= 100; ++index)
    {
      Vector2 mouseWorld = Main.MouseWorld;
      mouseWorld.X += (float) (index * 16 /*0x10*/);
      Projectile.NewProjectile(player.GetSource_ItemUse(this.Item, (string) null), Vector2.op_Subtraction(mouseWorld, new Vector2(0.0f, 320f)), Vector2.Zero, ModContent.ProjectileType<BuildIndicatorProjectile>(), 0, 0.0f, ((Entity) player).whoAmI, 0.0f, 0.0f, 0.0f);
    }
  }
}
