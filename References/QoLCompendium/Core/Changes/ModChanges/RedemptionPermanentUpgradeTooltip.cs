...
// Type: QoLCompendium.Core.Changes.ModChanges.RedemptionPermanentUpgradeTooltip
...
public class RedemptionPermanentUpgradeTooltip : GlobalItem
{
  ...
  public void UsedPermanentUpgrade(Item item, List<TooltipLine> tooltips)
  {
    ...
    if (item.type == Common.GetModItem(ModConditions.redemptionMod, "GalaxyHeart") && Redemption.BaseExtension.BaseExtension.Redemption(Main.LocalPlayer).galaxyHeart)
      Common.AddLastTooltip(tooltips, tooltip);
    if (item.type != Common.GetModItem(ModConditions.redemptionMod, "MedicKit") || !Redemption.BaseExtension.BaseExtension.Redemption(Main.LocalPlayer).medKit)
      return;
    Common.AddLastTooltip(tooltips, tooltip);
  }
}
