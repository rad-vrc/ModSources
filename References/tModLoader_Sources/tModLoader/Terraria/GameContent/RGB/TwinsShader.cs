...
namespace Terraria.GameContent.RGB
{
	...
	{
		// Token: 0x06004272 RID: 17010 RVA: 0x005F70A8 File Offset: 0x005F52A8
		...
		// Token: 0x06004273 RID: 17011 RVA: 0x005F710C File Offset: 0x005F530C
		...
		private void ProcessLowDetail(RgbDevice device, Fragment fragment, EffectDetailLevel quality, float time)
		{
			for (int i = 0; i < fragment.Count; i++)
			{
				...
				Vector4 value = Vector4.Lerp(this._veinColor, this._eyeColor, (float)Math.Sin((double)(time + canvasPositionOfIndex.X * 4f)) * 0.5f + 0.5f);
				...
				value = Vector4.Lerp(value, TwinsShader._irisColors[((gridPositionOfIndex.Y * 47 + gridPositionOfIndex.X) % TwinsShader._irisColors.Length + TwinsShader._irisColors.Length) % TwinsShader._irisColors.Length], dynamicNoise);
				fragment.SetColor(i, value);
			}
		}

		// Token: 0x06004274 RID: 17012 RVA: 0x005F71EC File Offset: 0x005F53EC
		[RgbProcessor(new EffectDetailLevel[]
		{
			1
		})]
			...
			for (int i = 0; i < fragment.Count; i++)
			{
				...
				else if (!flag)
				{
					...
					if (num4 < 0.7f)
					{
						...
						vector2 = Vector4.Lerp(vector2, this._flameColor, dynamicNoise);
					}
				}
