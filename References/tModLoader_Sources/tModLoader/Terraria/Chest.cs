...
namespace Terraria
{
	...
	public class Chest : IFixLoadedData
	{
		...
		/// </summary>
		// Token: 0x06000107 RID: 263 RVA: 0x00006744 File Offset: 0x00004944
		...
		// Token: 0x06000108 RID: 264 RVA: 0x0000676C File Offset: 0x0000496C
		...
		// Token: 0x06000109 RID: 265 RVA: 0x00006808 File Offset: 0x00004A08
		public static void Initialize()
		{
			int[] array = Chest.chestItemSpawn;
			int[] array3 = Chest.chestTypeToIcon;
			array3[0] = (array[0] = 48);
		...
		// Token: 0x0600010A RID: 266 RVA: 0x00007060 File Offset: 0x00005260
		...
		// Token: 0x0600010C RID: 268 RVA: 0x000070D5 File Offset: 0x000052D5
		public static bool IsLocked(int x, int y)
		{
			return Chest.IsLocked(x, y, Main.tile[x, y]);
		}
		...
		// Token: 0x0600010D RID: 269 RVA: 0x000070EC File Offset: 0x000052EC
		public unsafe static bool IsLocked(int x, int y, Tile t)
		{
			if (t == null)
			{
				return true;
			...
			{
				return TileLoader.IsLockedChest(x, y, (int)(*t.type));
			}
			return (*t.type == 21 && ((*t.frameX >= 72 && *t.frameX <= 106) || (*t.frameX >= 144 && *t.frameX <= 178) || (*t.frameX >= 828 && *t.frameX <= 1006) || (*t.frameX >= 1296 && *t.frameX <= 1330) || (*t.frameX >= 1368 && *t.frameX <= 1402) || (*t.frameX >= 1440 && *t.frameX <= 1474))) || (*t.type == 467 && *t.frameX / 36 == 13);
		}
		...
		/// </remarks>
		// Token: 0x0600010E RID: 270 RVA: 0x00007208 File Offset: 0x00005408
		...
		/// </returns>
		// Token: 0x0600010F RID: 271 RVA: 0x000072F0 File Offset: 0x000054F0
		public static Item PutItemInNearbyChest(Item item, Vector2 position)
		{
			...
			for (int i = 0; i < 8000; i++)
			{
				...
				{
					Vector2 vector;
					vector..ctor((float)(Main.chest[i].x * 16 + 16), (float)(Main.chest[i].y * 16 + 16));
					if ((vector - position).Length() < 600f)
					{
						for (int j = 0; j < Main.chest[i].item.Length; j++)
						{
							...
							else if (item.IsTheSameAs(Main.chest[i].item[j]))
							{
								flag2 = true;
								int numTransfered;
								if (ItemLoader.TryStackItems(Main.chest[i].item[j], item, out numTransfered, false))
								{
									if (numTransfered > 0)
									{
		...
		// Token: 0x06000110 RID: 272 RVA: 0x000074C0 File Offset: 0x000056C0
		...
		// Token: 0x06000111 RID: 273 RVA: 0x00007500 File Offset: 0x00005700
		...
		// Token: 0x06000112 RID: 274 RVA: 0x00007559 File Offset: 0x00005759
		public object Clone()
		{
		...
		// Token: 0x06000113 RID: 275 RVA: 0x00007564 File Offset: 0x00005764
		public unsafe static bool Unlock(int X, int Y)
		{
			...
			if (*Main.tile[X, Y].type >= TileID.Count)
			{
				...
				type = 11;
				bool manual = false;
				if (!TileLoader.UnlockChest(X, Y, (int)(*Main.tile[X, Y].type), ref num, ref type, ref manual))
				{
					return false;
				}
				if (manual)
			...
			for (int i = X; i <= X + 1; i++)
			{
				for (int j = Y; j <= Y + 1; j++)
				{
					...
					{
						ref short frameX = ref tileSafely2.frameX;
						frameX -= num;
						for (int k = 0; k < 4; k++)
						{
		...
		// Token: 0x06000114 RID: 276 RVA: 0x000077A0 File Offset: 0x000059A0
		public unsafe static bool Lock(int X, int Y)
		{
			if (Main.tile[X, Y] == null || Main.tile[X + 1, Y] == null || Main.tile[X, Y + 1] == null || Main.tile[X + 1, Y + 1] == null)
			{
				return false;
			...
			if (*Main.tile[X, Y].type >= TileID.Count)
			{
				num = 36;
				bool manual = false;
				if (!TileLoader.LockChest(X, Y, (int)(*Main.tile[X, Y].type), ref num, ref manual))
				{
					return false;
				}
				if (manual)
		...
		/// <remarks>
		/// The Chest is stored in the top-left corner of the chest tile. If you have an arbitrary chest tile, you may need to use <see cref="M:Terraria.ObjectData.TileObjectData.TopLeft(System.Int32,System.Int32)" /> to find the top left coordinates to use in this method.
		/// </remarks>
		// Token: 0x06000116 RID: 278 RVA: 0x000079A4 File Offset: 0x00005BA4
		...
		/// <summary>
		/// Similar to <see cref="M:Terraria.Chest.FindChest(System.Int32,System.Int32)" /> except it also succeeds for tile coordinates directly above or to the left of the Chest location.
		/// <br /><br /> <b>The logic in this method is incorrect and this method should not be used.</b> It is recommended to use <see cref="M:Terraria.Chest.FindChest(System.Int32,System.Int32)" /> with <see cref="M:Terraria.ObjectData.TileObjectData.TopLeft(System.Int32,System.Int32)" /> if you do not have the top left tile coordinate.
		/// </summary>
		// Token: 0x06000117 RID: 279 RVA: 0x000079EC File Offset: 0x00005BEC
		...
		/// </returns>
		// Token: 0x06000118 RID: 280 RVA: 0x00007A58 File Offset: 0x00005C58
		...
		// Token: 0x06000119 RID: 281 RVA: 0x00007AA0 File Offset: 0x00005CA0
		public unsafe static bool NearOtherChests(int x, int y)
		{
		...
		/// </returns>
		// Token: 0x0600011A RID: 282 RVA: 0x00007AF4 File Offset: 0x00005CF4
		public static int AfterPlacement_Hook(int x, int y, int type = 21, int style = 0, int direction = 1, int alternate = 0)
		{
			...
			int num = Chest.FindEmptyChest((int)baseCoords.X, (int)baseCoords.Y, 21, 0, 1, 0);
			if (num == -1)
			{
				return -1;
			}
		...
		/// </remarks>
		// Token: 0x0600011B RID: 283 RVA: 0x00007C1C File Offset: 0x00005E1C
		...
		// Token: 0x0600011C RID: 284 RVA: 0x00007C94 File Offset: 0x00005E94
		...
		/// <param name="Y">The y-coordinate to check, in tile coordinates.</param>
		/// <returns><see langword="false" /> if a chest exists at (<paramref name="X" />, <paramref name="Y" />) and if any element in that chest's <see cref="F:Terraria.Chest.item" /> array is not <see cref="P:Terraria.Item.IsAir" />. Otherwise, returns <see langword="true" />.</returns>
		// Token: 0x0600011D RID: 285 RVA: 0x00007D0C File Offset: 0x00005F0C
		...
		// Token: 0x0600011E RID: 286 RVA: 0x00007DBC File Offset: 0x00005FBC
		...
		// Token: 0x0600011F RID: 287 RVA: 0x00007E40 File Offset: 0x00006040
		public int AddItemToShop(Item newItem)
		{
			...
			if (num >= newItem.stack)
			{
				for (int i = 0; i < 39; i++)
				{
					if (this.item[i] != null && this.item[i].type == newItem.type)
					{
						return i;
					}
				}
			...
			for (int j = 0; j < 39; j++)
			{
				if (this.item[j] == null || this.item[j].type == 0 || j == 38)
				{
					this.item[j] = newItem.Clone();
					this.item[j].favorited = false;
					this.item[j].buyOnce = true;
					this.item[j].stack -= num;
					int value = this.item[j].value;
					return j;
				}
			}
			return 0;
		...
		// Token: 0x06000120 RID: 288 RVA: 0x00007F10 File Offset: 0x00006110
		...
		// Token: 0x06000121 RID: 289 RVA: 0x00008168 File Offset: 0x00006368
		...
		// Token: 0x06000122 RID: 290 RVA: 0x000081BC File Offset: 0x000063BC
		...
		// Token: 0x06000123 RID: 291 RVA: 0x0000844C File Offset: 0x0000664C
		...
		// Token: 0x06000124 RID: 292 RVA: 0x000084D4 File Offset: 0x000066D4
		...
		/// </summary>
		// Token: 0x06000125 RID: 293 RVA: 0x00008A20 File Offset: 0x00006C20
		public static void SetupTravelShop()
		{
			...
			if (Main.hardMode)
			{
				...
				{
					num2++;
					Chest.SetupTravelShop_AdjustSlotRarities(num2, ref rarity);
			...
			while (added < num)
			{
				...
				{
					Chest.SetupTravelShop_AddToShop(it2, ref added, ref count);
				}
			}
			rarity = array;
			num2 = 0;
			...
			while (num2 < 5000)
			{
				...
				{
					Chest.SetupTravelShop_AddToShop(it3, ref added, ref count);
					break;
				}
			}
		...
		// Token: 0x06000126 RID: 294 RVA: 0x00008BE2 File Offset: 0x00006DE2
		public void SetupShop(int type)
		{
			this.SetupShop(NPCShopDatabase.GetShopNameFromVanillaIndex(type), Main.LocalPlayer.TalkNPC);
		}
		...
		internal void VanillaSetupShop(int type)
		{
			...
			case 18:
			{
				...
				for (int num6 = 0; num6 < 54; num6++)
				{
					...
					{
						num5 += (long)Main.player[Main.myPlayer].inventory[num6].stack;
					}
					if (Main.player[Main.myPlayer].inventory[num6].type == 72)
					{
						num5 += (long)(Main.player[Main.myPlayer].inventory[num6].stack * 100);
					}
					if (Main.player[Main.myPlayer].inventory[num6].type == 73)
					{
						num5 += (long)(Main.player[Main.myPlayer].inventory[num6].stack * 10000);
					}
					if (Main.player[Main.myPlayer].inventory[num6].type == 74)
					{
						num5 += (long)(Main.player[Main.myPlayer].inventory[num6].stack * 1000000);
					}
				}
				if (num5 >= 1000000L)
				{
			...
			case 19:
				...
				{
					if (Main.travelShop[num7] != 0)
					{
						array[num].netDefaults(Main.travelShop[num7]);
						num++;
					}
				}
				break;
			...
			bool flag5 = type != 19 && type != 20;
			bool flag3 = TeleportPylonsSystem.DoesPositionHaveEnoughNPCs(2, Main.LocalPlayer.Center.ToTileCoordinates16());
			if (flag5 && (flag || Main.remixWorld) && flag3 && !Main.player[Main.myPlayer].ZoneCorrupt && !Main.player[Main.myPlayer].ZoneCrimson)
			{
		...
		// Token: 0x06000128 RID: 296 RVA: 0x0000CECC File Offset: 0x0000B0CC
		public void SetupShop(string shopName, NPC npc)
		{
			Array.Fill<Item>(this.item, null);
			AbstractNPCShop shop;
			if (NPCShopDatabase.TryGetNPCShop(shopName, out shop))
			{
				bool overflow;
				shop.FillShop(this.item, npc, out overflow);
				if (overflow)
				{
					Main.NewText(Language.GetTextValue("tModLoader.ShopOverflow"), new Color?(Color.Orange));
			...
			Span<Item> span = this.item.AsSpan<Item>();
		...
		// Token: 0x06000129 RID: 297 RVA: 0x0000CF7D File Offset: 0x0000B17D
		...
		// Token: 0x0600012A RID: 298 RVA: 0x0000CFAA File Offset: 0x0000B1AA
		...
		/// </summary>
		/// <param name="worldPosition">The position of the chest to ask, in world coordinates.</param>
		/// <param name="duration">The duration of the eating animation in ticks.</param>
		// Token: 0x0600012B RID: 299 RVA: 0x0000CFCC File Offset: 0x0000B1CC
		...
		/// </summary>
		// Token: 0x0600012C RID: 300 RVA: 0x0000D004 File Offset: 0x0000B204
		...
		/// </summary>
		// Token: 0x0600012D RID: 301 RVA: 0x0000D178 File Offset: 0x0000B378
