...
using TranslateTest2.Common.Players; // SPModPlayer

namespace TranslateTest2.Common.GlobalProjectiles
{
    public class SPGlobalProj : GlobalProjectile
    {
        ...
        public override void OnSpawn(Projectile projectile, IEntitySource source)
        {
            if (source is EntitySource_ItemUse itemUse && (projectile.minion || projectile.sentry) && itemUse.Item.isMinionSummonItem())
            {
                ...
                deviation = itemUse.Item.prefix == ModContent.PrefixType<Content.Prefixes.Deviation>();
                echo = itemUse.Item.prefix == ModContent.PrefixType<Content.Prefixes.Echo>();
                blessing = itemUse.Item.prefix == ModContent.PrefixType<Content.Prefixes.Blessing>();
                contract = itemUse.Item.prefix == ModContent.PrefixType<Content.Prefixes.Contract>();
                UpdateCounter = 0f;
            ...
            if (source is EntitySource_Parent parent)
            {
                ...
                if (parent.Entity is Projectile p2 && (p2.minion || p2.sentry))
                {
                    ...
                    if (pg.echo && EchoDoubleProj && Utils.NextFloat(Main.rand) < Content.Prefixes.Echo.DoubleChance)
                    {
    ...
    // 原典では Electrified の追加平坦ダメージ修正は存在しないため削除
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            {
                int laserDamage = (int)(electrifiedPlayer.statLifeMax2 * Content.Prefixes.Electrified.DmgMultByPlayerMaxHealth);
                Projectile.NewProjectile(projectile.GetSource_FromThis(), projectile.Center, Vector2.Zero, ModContent.ProjectileType<Content.Projectiles.ElectricLaser>(), laserDamage, 0.0f, electrifiedPlayer.whoAmI, electrifiedPlayer.Center.X, electrifiedPlayer.Center.Y, 0.0f);
            }
