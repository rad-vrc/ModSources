...
using Terraria.UI;
// using Terraria.GameInput; // PlayerInput は Terraria に含まれるため重複 using を避ける
using TranslateTest2.Content.Items.Tools; // WarpMode
using Terraria.GameContent.UI.Elements; // UIPanel
...
namespace TranslateTest2.Common.UI
{
    ...
    UIPanel inputHitBox;         // 入力ブロック用の不可視パネル（ガイド推奨の親要素）
    ...
    KeyboardState _prevKeyState; // 前フレームのキー状態（Q/Eのエッジ検出用）
        ...
        public override void OnInitialize()
        {
            ...
            Append(anchor);
            // 円周UIを包む十分なサイズの不可視パネルを追加（入力ブロック/スクロールロック用）
            inputHitBox = new UIPanel();
            inputHitBox.BackgroundColor = Color.Transparent;
            inputHitBox.BorderColor = Color.Transparent;
            inputHitBox.Width.Set(280f, 0f);  // 半径90 + 余白をカバー
            inputHitBox.Height.Set(280f, 0f);
            inputHitBox.HAlign = 0.5f;
            inputHitBox.VAlign = 0.5f;
            Append(inputHitBox);
        ...
        protected override void DrawSelf(SpriteBatch sb)
        {
            ...
            // 入力ブロック（ガイド準拠）
            if (inputHitBox != null && inputHitBox.ContainsPoint(Main.MouseScreen))
            {
        ...
        {
            string path = $"TranslateTest2/Content/Items/Tools/AiPhone_{mode}";
            if (ModContent.HasAsset(path))
                return ModContent.Request<Texture2D>(path).Value;
            return ModContent.Request<Texture2D>("TranslateTest2/Content/Items/Tools/AiPhone_Dummy").Value;
        }
}
}
