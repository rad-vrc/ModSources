...
namespace TranslateTest2.Common.Players
{
    public class InventoryPlayer : ModPlayer
    {
        ...
        private bool HandleLeftClick(Item[] inventory, int context, int slot)
        {
            ...
            if (release != leftClickCache) TranslateTest2.DebugInChat($"mouseLeftRelease == leftClickCache ({release == leftClickCache})");
            if (release || AndroLib.PreventDoubleClickInJourneyMode(context, overrideShiftLeftClick))
            {
                overrideShiftLeftClick = false;
                TranslateTest2.DebugInChat($"vanilla left click context: {context}, slot: {slot} item: {inventory[slot].type}");
                return false;
            }
            var left = InventoryConfig.Instance.LeftMouse;
            if (!left.Enabled || !left.ModifierOptions.IsSatisfied(Player)) return false;
            TranslateTest2.DebugInChat($"custom left click context: {context}, slot: {slot} item: {inventory[slot].type}");
        ...
        private bool HandleRightClick(Item[] inventory, int context, int slot)
        {
            ...
            {
                TranslateTest2.DebugInChat($"vanilla right click context: {context}, slot: {slot} release: {Main.mouseRightRelease} cache: {rightClickCache}");
                return false;
            }
            var right = InventoryConfig.Instance.RightMouse;
            if (!right.Enabled || !right.ModifierOptions.IsSatisfied(Player)) return false;
            TranslateTest2.DebugInChat($"custom right click context: {context}, slot: {slot} item: {inventory[slot].type}");
