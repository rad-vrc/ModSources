...
namespace TranslateTest2.Map {
  public class AiPhoneMapLayer : ModMapLayer {
    ...
    private static Texture2D PinTex => _pinTex ??= ModContent.Request<Texture2D>("TranslateTest2/Items/Tools/AiPhone_Dummy").Value;
  ...
  var drawResult = ctx.Draw(pinTex, tilePos, Color.White, new SpriteFrame(1, 1, 0, 0), 0.9f, 1.1f, Terraria.UI.Alignment.Center);
    if (drawResult.IsMouseOver) {
      ...
      Items.Tools.AiPhone.TeleportTo(me, pl.Center);
        }
      }
      // タウンNPC
  ...
  var drawResult = ctx.Draw(pinTex, tilePos, Color.White, new SpriteFrame(1, 1, 0, 0), 0.9f, 1.1f, Terraria.UI.Alignment.Center);
    if (drawResult.IsMouseOver) {
          if (Main.mouseLeft && Main.mouseLeftRelease)
      Items.Tools.AiPhone.TeleportTo(me, npc.Center + new Vector2(32, 0));
        }
      }
    }
    static bool HasAiPhone(Player p) {
      for (int i = 0; i < p.inventory.Length; i++)
  if (p.inventory[i].type == ModContent.ItemType<Items.Tools.AiPhone>()) return true;
      return false;
    }
  }
}
