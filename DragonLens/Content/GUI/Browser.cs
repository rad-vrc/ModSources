...
namespace DragonLens.Content.GUI
{
	...
	internal abstract class Browser : DraggableUIState
	{
		...
		public override void Draw(SpriteBatch spriteBatch)
		{
			...
			// 新規: 見失い防止のアウトライン
			GUIHelper.DrawOutline(spriteBatch, target, ThemeHandler.currentColorProvider.SafeOutlineColor);
	...
	internal abstract class BrowserButton : SmartUIElement
	{
		...
		{
			// 日本語IMEでも安定するよう、大文字小文字無視の部分一致に変更
			bool matches = (Identifier ?? string.Empty).IndexOf(parent.searchBar.currentValue ?? string.Empty, StringComparison.OrdinalIgnoreCase) >= 0;
			if (!matches || parent.ShouldBeFiltered(this))
			{
				Width.Set(0, 0);
				Height.Set(0, 0);
