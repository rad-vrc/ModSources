	...
	internal class AssetBrowser : Browser
	{
		...
		public override void PopulateGrid(UIGrid grid)
		{
			...
			foreach (Mod mod in ModLoader.Mods)
			{
				...
				foreach (KeyValuePair<string, IAsset> pair in EnumerateAssets(repo))
				{
		...
		private static IEnumerable<KeyValuePair<string, IAsset>> EnumerateAssets(AssetRepository repo)
		{
			// まずはメソッドが存在すればそれを使う（将来版）
			var getAssets = repo.GetType().GetMethod("GetAssets", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (getAssets != null)
			{
				if (getAssets.Invoke(repo, null) is IEnumerable<KeyValuePair<string, IAsset>> seq)
					return seq;
			}

			// 旧版: 非公開フィールド _assets にフォールバック
			var field = repo.GetType().GetField("_assets", BindingFlags.Instance | BindingFlags.NonPublic);
			if (field != null && field.GetValue(repo) is IDictionary<string, IAsset> dict)
				return dict;

			return Array.Empty<KeyValuePair<string, IAsset>>();
		}
	}
			...
			if (mod.RootContentSource is TModContentSource)
			{
				...
				if (Path.Exists(path))
				{
					using Stream stream = File.OpenRead(path);
					{
						...
						// 1.4.4では ownValue は内部。即時差し替えはスキップ（通知のみ）。
	...
	internal class ShaderAssetButton : BrowserButton
	{
		...
		public void ReloadFromBinary()
		{
			...
			if (xnbHeader[0] == 'X' &&
				...
				xnbHeader[2] == 'B')
			{
				// 内部API変更により、ここでの直接再ロードはサポート外。安全に早期リターン。
				return;
			}
		}
